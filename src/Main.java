...
public class Main {
    static int width = 1500, height = 800;
    static JFrame frame = new JFrame();
    static int num = -1;// Номер машины, которая победила
    ...
    public static class RunCar extends Thread {
        ...
        public void run() {
            ...
            while (num == -1) {
                try {
                    Thread.sleep(4);
                    car.l.setBounds(car.l.getX()+(int) (Math.random() * 5), car.l.getY(), car.l.getWidth(), car.l.getHeight()); // Перемещаем случайно машину не более чем на 5 символов по горизонтали
    ...
    public static void main(String[] args) throws IOException {
        ...
        frame.setTitle("Великая гонка тысячелетия!");
        Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
        frame.setBounds(dim.width / 2 - width / 2, dim.height / 2 - height / 2, width, height + 30);
        frame.getContentPane().setLayout(null);
        frame.getContentPane().setBackground(Color.black);
        BufferedImage im = ImageIO.read(new URL("https://wallpaperstock.net/wallpapers/thumbs/32635.jpg"));
